...
// Horizontal (perpendicular) tracking wheel: Rotation sensor on port 6, 3.25" wheel.
// distance_to_center -7.61" (Measure Offsets value; +7.61 ran X/Y away on a spin).
ez::tracking_wheel horiz_tracker(6, 3.25, -7.61);   // This tracking wheel is perpendicular to the drive wheels

// Vertical (parallel) tracking wheel: Rotation sensor on port 15, reversed via the negative port.
// distance_to_center 1.05" (Left value from "Measure Offsets").
ez::tracking_wheel vert_tracker(-15, 2.75, 1.05);   // This tracking wheel is parallel to the drive wheels
...
void initialize() {
  ...
  chassis.odom_tracker_back_set(&horiz_tracker);  // Horizontal tracker (behind center) -> full 2D odom
  // Look at your vertical tracking wheel and decide if it's to the left or right of the center of the robot
  //  - change `left` to `right` if the tracking wheel is to the right of the centerline
  //  - ignore this if you aren't using a vertical tracker
  chassis.odom_tracker_left_set(&vert_tracker);  // Enables odometry using our vertical tracker
  ...
  // descore is always up -- deploy it at startup and never lower it.
  descore.extend();
...
void opcontrol() {
  ...
  while (true) {
    ...
    // . . .
    intake_control();  // R1 in / R2 reverse (both motors); L1 hold = hood stopper down
